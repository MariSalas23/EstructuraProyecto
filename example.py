...
class LibraryApp:
    def __init__(self, root):
        ...
        inner_filters_frame.pack(fill="both", expand=True, padx=(10, 0), pady=5)  # Cambiado a 10px
        
        filters_title = tk.Label(inner_filters_frame, text="Filtros", font=custom_font, bg=frame_bg, fg=border_color)
        filters_title.grid(row=0, column=0, pady=(0, 20), sticky="ew", padx=(10, 0))  # Ajustar padding a la derecha
        
        # Clasificar por
        classify_label = tk.Label(inner_filters_frame, text="Clasificar por:", font=custom_font2, bg=frame_bg)
        classify_label.grid(row=1, column=0, sticky="w", pady=(0, 5), padx=(20, 0))  # Cambiado a 10px
        
        self.classify_combo = ttk.Combobox(inner_filters_frame, values=["Año", "Autor"], state="readonly")
        self.classify_combo.grid(row=2, column=0, padx=(20, 0), pady=5, sticky="ew")  # Cambiado a 10px
        ...
        genre_button = tk.Button(inner_filters_frame, text="Seleccionar Género", command=self.select_genre,
                                 ...
                                 activeforeground="white", relief="flat")
        genre_button.grid(row=3, column=0, pady=(20, 20), sticky="ew", padx=(20, 0))  # Cambiado a 10px
        
        # Ordenar por
        sort_label = tk.Label(inner_filters_frame, text="Ordenar por:", font=custom_font2, bg=frame_bg)
        sort_label.grid(row=4, column=0, sticky="w", pady=(20, 5), padx=(20, 0))  # Cambiado a 10px
        
        asc_button = tk.Button(inner_filters_frame, text="Menor a mayor", command=self.sort_asc,
                               bg=button_bg, fg=button_fg, font=custom_font2, activebackground=button_active_bg,
                               activeforeground="white", relief="flat")
        asc_button.grid(row=5, column=0, pady=5, sticky="ew", padx=(20, 0))  # Cambiado a 10px
        
        desc_button = tk.Button(inner_filters_frame, text="Mayor a menor", command=self.sort_desc,
                                bg=button_bg, fg=button_fg, font=custom_font2, activebackground=button_active_bg,
                                activeforeground="white", relief="flat")
        desc_button.grid(row=6, column=0, pady=5, sticky="ew", padx=(20, 0))  # Cambiado a 10px

    def search_books(self):
        # Implementación de búsqueda pendiente
        pass

    def sort_asc(self):
        # Implementación de orden ascendente pendiente
        pass

    def sort_desc(self):
        # Implementación de orden descendente pendiente
        pass
